...
For this reason, I do not include rigorous security proofs of any of these constructs.  Instead I show only $correctness$ of the verification, which is analogous to a completeness security proof: this shows that a prover $with$ a valid witness can demonsrate that to a verifier.  I do not attempt to show $soundness$, which is that a prover $without$ a valid witness cannot so convince the verifier; nor do I show $zero$ $knowledge$, which is that the proof transcript does not leak any information $about$ the witness.  These proofs are difficult and non-intuitive, and would not help the target audience at this stage of their learning.
