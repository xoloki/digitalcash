...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{digitalcash-black-white-crop.jpg}
\end{figure}
...
Group theory is an abstraction of this idea, and can be applied to many contexts where we do not have numbers modulo some fixed modulus.  A group consists of a set, and a binary operation on pairs of members of that set that returns another member of the set.  You can think of this operation as an abstract version of multiplication.  

Numbers modulo some fixed modulus form a group; you can multiply two of those numbers, take the remainder modulo the modulus, and you will get another member of the same set.  But you can also use elliptic curves, polynomials, or even more exotic sets.

We can do the group operation repeatedly some number of times on a single member of the set.  This is an abstraction of the idea of exponentiation, raising some number to a power.  And we can also try to reverse exponentiation; as with finite fields, this requires brute force, and makes public key cryptography safe.  But unlike with finite fields, we can do this with any group.  For a variety of technical reasons, we normally use elliptic curves as our group set, and elliptic curve addition as our group operation.

Some members of a group, when exponentiated, will produce every member of the set exactly once.  These members are called generators.  The number of times you can do this exponentiation before returning to the same value is called the $order$ of a group.
