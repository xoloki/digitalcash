...
Consider a crate of eggs that holds a dozen eggs.  Using discrete math, we could ask the question of how many dozens can be filled by a particular number of eggs, and how many were left over once we had fully filled as many as possible.
...
Keeping with our egg crate example, as noted above the remainders cycle between zero and one less than the size of the crate, aka the modulus.  If the modulus is prime, we name these sets of remainders finite fields.

This means that there is a fixed number of remainders for any field, and that we can do the basic mathematical operations of addition and multiplication on them. We can also take inverses, both additively and multiplicatively.  Subtraction and division are thus also possible, by taking inverses then applying the operation.

If we take a member of a finite field, and multiply it by itself repeatedly, this is what is called exponentiation.  Exponentiation can be reversed; rather than finding the result of repeated multiplication, we decipher how many times multiplication would need to be done to get a particular result.  This number is called a logarithm, and is the reverse of exponentiation.
...
Group theory is a simplified abstraction of this idea, that can be used for constructs other than numbers modulo a prime modulus.  A group consists of a set, and a single binary operation on pairs of elements of that set that returns another element of the set.  You can think of this operation as an abstract version of multiplication.

Numbers modulo some fixed modulus form a group; you can multiply two of those numbers, take the remainder modulo the modulus, and you will get another element of the same set.  But you can also use elliptic curves, polynomials, or even more exotic sets.

We can do the group operation repeatedly some number of times on a single element of the set.  This is an abstraction of the idea of exponentiation, raising some number to a power.  And we can also try to reverse exponentiation; as with finite fields, this requires brute force, and makes public key cryptography safe.  But unlike with finite fields, we can do this with any group.  For a variety of technical reasons, we normally use elliptic curves as our group set, and elliptic curve addition as our group operation.

Some elements of a group, when exponentiated, will produce every element of the set exactly once.  These elements are called generators.  The number of times you can do this exponentiation before returning to the same value is called the $order$ of a group.  Since exponentiation more times than the order of a group is cyclic, we use numbers modulo the group order for this operation as well.  These numbers are referred to as group scalars, to reflect this fact.

If the order of a group is prime, then all elements of the group are generators.  
...
\begin{align}
  c_{j+1} = H_s(L_{j,0}, L_{j,0}, ... , L_{j,0}, L_{j,0}, G^{r_{j,m}} y^{c_j})
\end{align}
