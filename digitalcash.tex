...
But while it is trivial to find logarithms in non-discrete math, it is not generally possible to find discrete logarithms without resorting to brute force.  The difficulty of this problem, called the Discrete Logarithm Problem, or DLP, is what makes much of public key cryptography work.

\subsubsection{Group Theory}

Finite fields are usually represented by the remainders of numbers divides by some fixed modulus.  But this is not the only way to do such operations.

Group theory is an abstraction of this idea, and can be applied to many contexts where we do not have numbers modulo some fixed modulus.  A group consists of a set, and a binary operation on pairs of elements of that set that returns another element of the set.  You can think of this operation as an abstract version of multiplication.  
...
So if $R = G^s X^c$, then the prover must have known $x$.  Otherwise the prover would not have been able to construct a proper $s$, allowing the equation to balance.  The tuple $(R,s)$ is thus a proof of ownership of $X$.
...
Then we can calculate $L_j$ and $R_j$ as we did for all $L_i$ and $R_i$, using $r_j$ and $c_j$, and the values should be equal to the original $L_j$ and $R_j$ calculated using $v$. The prover now sends $(I, X_0, s_0, …, X_{n-1}, s_{n-1}, c_0)$ to the verifier.  The verifier can reconstruct all $(c_i, L_i, R_i)$ and then check that $c_0 = c_n$.
...
The MLSAG solves this by using key vectors rather than single keys \cite{ringct}.  So key $X_i$ is instead a vector of $m$ keys
\begin{align}
  X_i = (X_{i,0}, ..., X_{i,m-1})
\end{align}

As before, there exists a secret index $j$, for which we know the private keys to each public key.  Each $s_i$ is now also a vector, and
\begin{align}
  s_i = (s_{i,0}, ..., s_{i,m-1})
\end{align}
...
The prover proceeds as with an LSAG, starting with index $j$.  $r$ is now a vector of $m$ elements.  The $L_{j,k}$ and $R_{j,k}$ entries are calculated in the same way, but using $X_{j,k}$ and $r_k$.  $c_{j+1}$ is calculated using all $L_{j,k}$ and $R_{j,k}$:
\begin{align}
  L_{j,k} &= G^{r_k}\\
...
\begin{align}
  L_{i,k} &= G^{r_{i,k}} X_{i,k}^{c_i}\\
  R_{i,k} &= H_g(y_i)^{r_{i,k}} I^{c_i}\\
...
The prover sends the same set of $(I, X_0, s_0, ..., X_{m-1}, s_{m-1}, c_0)$ as for LSAG, but each $y_i$ and $s_i$ is now a vector.  The verifier again calculates the full set of $c_i$, with corresponding $L_i$ and $R_i$, and checks that $c_0 = c_n$.
...
The creator then uses this, with the destination public key $(A, B)$, to construct the one time address $X$.  This address is a public key, with an associated private key $x$:
\begin{align}
  X = G^x = G^{H_s(rA)} B
\end{align}
