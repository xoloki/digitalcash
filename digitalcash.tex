...
A Schnorr proof allows us to prove ownership of the private key $x$ for a public key $X = G^x$, but what if we want to use that key as a signature on some message $M$?  This message could be anything, such as an email, or a cryptocurrency transaction, or more generally a hash of some data $D$.  We must tie the message to the proof in order to form a signagure.
...
A digital signature allows us to sign a message for a public key, proving we know the corresponding private key.  But the signature is tied to a single key, which is bad for privacy.  It would be better to construct a one out of many signature, to prove that we knew the private key $x$ for one out of $N$ public keys $(X_1, ..., X_N)$.  Such a signature is called a ring signature \cite{cryptonote}.
...
\begin{align}
  c_1 = H_s(G^r, G^{s_2} X_1^{c_2}, M) - c_2
\end{align}

This is equivalent algebraically to:
\begin{align}
  c_1 + c_2 = H_s(G^r, G^{s_2} X_1^{c_2}, M)\nonumber
\end{align}
...
Since $G^r = G^{s_1} X^{c_1}$, the two hashes are identical.  Thus 
\begin{align}
  c_1 + c_2 = H_s(G^{s_1} X^{c_1}, G^{s_2} X_1^{c_2}, M)
\end{align}
