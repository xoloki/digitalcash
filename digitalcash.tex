...
For this reason, we do not include rigorous security proofs of any of these constructs.  Instead we show only $correctness$ of the verification, which is analogous to a completeness security proof: this shows that a prover $with$ a valid witness can demonsrate that to a verifier.  We do not attempt to show $soundness$, which is that a prover $without$ a valid witness cannot so convince the verifier; nor do we show $zero$ $knowledge$, which is that the proof transcript does not leak any information $about$ the witness.  These proofs are difficult and non-intuitive, and would not help the target audience at this stage of their learning.
...
Keeping with our egg crate example, as noted above the remainders cycle between zero and one less than the size of the crate, aka the modulus.  We name these sets of remainders finite fields.

This means that there is a fixed number of remainders for any field, and that we can do many typical mathematical operations on them: addition, subtraction, and multiplication are easily possible. Division is also possible on some fields; it often takes the form of finding a multiplicitive inverse, then multiplying with it rather than doing division per se.

If we take a member of a finite field, and multiply it by itself repeatedly, this is what is called exponentiation.  Exponentiation can be reversed; rather than finding the result of repeated multiplication, we deciper how many times multiplication would need to be done to get a particular result.  This number is called a logarithm, and is the reverse of exponentiation.

But while it is trivial to find logarithms in non-discrete math, it is not generally possible to find discrete logarithms without resorting to brute force.  The difficults of this problem, called the Discrete Logarithm Problem, or DLP, is what makes public key cryptography work.
...
Some elementss of a group, when exponentiated, will produce every element of the set exactly once.  These element are called generators.  The number of times you can do this exponentiation before returning to the same value is called the $order$ of a group.  Since exponentiation more times than the order of a group is cyclic, we use numbers modulo the group order for this operation as well.  These number are referred to as group scalars, to reflect this fact.
...
We will use lowercase variable names for group scalars ($x$), and uppercase for group elements ($X$).  Groups themselves will be denoted as $\mathbb{G}$, which we will also use for sets as in $\mathbb{S}$.  Though we will be expressing all of the math here in terms of exponentials, we borrow naming conventions from elliptic curves to make things explcit.  A public key $X$ is defined as $G^x$ for some generator $G$.
...
When dealing with groups, we often want to hash some data, then convert the ouput bits to a group element or scalar.  This is trivial in the case of a scalar; simly hash to the same number of bits as the scalar, then reduce modulo the group order.  We denote this function as $H_s$.
...
This is problematic, though, because now there is a well known discrete logarith for $H_g(d)$, namely $H_s(d)$.  The existence of this discrete logarithm breaks the assumptions needed to prove the security of algorithms that require $H_g$.  So we must use more complex algorithms; the details are out of scope for this document, but are well known and easily found.
...
So if $R = G^s X^c$, then the prover must have known $x$.  Otherwise the prover would not have been able to construct a proper $s$, allowing the equation to balance.  The tuple $(R,s)$ thus forms the proof of ownership of $X$.

\newpage
\section{Digital Signatures}
A Schnorr proof allows us to prove ownership of the private key $x$ for a public key $X = G^x$, but what if we want to use that key to sign some message $M$?  This message could be anything, such as an email or a cryptocurrency transaction, but generally will be hash of some data $D$.  We must tie the message to the proof in order to form a signagure.
