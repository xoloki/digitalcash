...
There are many papers which outline, define, use, mutate, expand on, or cite the following techniques.  But these papers invariably use different terminology, swapping variable names and using various subsets as needed.  In this document, we will endeavor to be both thorough and strongly consistent, which should not only make the math easier to understand, but also expose the deep connections and demonstrate how each piece builds on the previous.

The target audience here is not mathematicians or cryptographers, but rather engineers with a reasonable mathematical background who want to understand the constructs that underpin modern cryptocurrencies.  This will allow them to become much more effective engineers in this problem domain, and perhaps some will be inspired to learn even more.

For this reason, we do not include rigorous security proofs for any of these constructs.  Instead we show only correctness of the verification, which is analogous to a $completeness$ security proof: this shows that a prover $with$ a valid witness $can$ demonstrate that to a verifier.  We do not attempt to show $soundness$, which is that a prover $without$ a valid witness $cannot$ so convince the verifier; nor do we show $zero$ $knowledge$, which is that the proof transcript does not leak any information $about$ the witness.  These proofs are difficult and non-intuitive, and would not help the target audience at this stage of their learning.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{digitalcash.jpg}
\end{figure}

\newpage
\section{Background}

\subsection{Discrete Math}

Much of mathematics deals with quantities that vary smoothly and fractionally between values, such as temperature or position; these are called $continuous$.  But another branch of mathematics deals with values which represent whole quantities; these are called $discrete$, such as the number of eggs in a crate, or people on a boat.  Cryptography uses discrete math almost exclusively.

\subsubsection{Modular Arithmetic}

Consider a crate of eggs that holds a fixed number of eggs.  Using discrete math, we could ask the question of how many crates can be filled by a particular number of eggs, and how many were left over once we had fully filled as many as possible.  If the number of eggs left over is zero, we say that the size of the crate $divides$ the number of eggs.

Because the crates are filled one at a time, and the remainder cycles between zero and one less than the size of the crate, we call this kind of math modular arithmetic.  The crates are the modules; we in fact seldom care much about the number of crates filled, but rather the remainders.  We refer to the size of the crate as the modulus, and it determines how many remainders are possible.

These remainders form a finite set, and we can do the basic mathematical operations of addition and multiplication on elements of this set. We can also take additive inverses, which allow us to do subtraction.  And crucially, if the modulus is prime, we can take multiplicative inverses, and division is thus also possible.

If we take an element of such a set, and multiply it by itself repeatedly, this is what is called exponentiation.  Exponentiation can be reversed; rather than finding the result of repeated multiplication, we decipher how many times multiplication would need to be done to get a particular result.  This number is called a logarithm, and is the reverse of exponentiation.

But while it is trivial to find logarithms in non-discrete math, it is not generally possible to find discrete logarithms without resorting to brute force.  The difficulty of this problem, called the Discrete Logarithm Problem (DLP), is what makes much of public key cryptography secure.

\subsubsection{Group Theory}

Group theory is an abstraction of arithmetic, that can be used for constructs other than discrete numbers.  A group consists of a set, and a single binary operation on elements of the set that returns another element of the set. You can think of this operation as an abstract version of multiplication.  There must also be an identity element, which when applied to any element will result in the element itself; and for every element, there must be an inverse, such that applying it to that element produces the identity.  For cryptography, we are generally only interested in finite groups, whose behavior is similar to modular arithmetic.

Numbers modulo some fixed modulus form a group; you can multiply two of those numbers, take the remainder modulo the modulus, and you will get another element of the same set.  But you can also use elliptic curves, polynomials, or even more exotic sets, with any operation you choose.

We can do the group operation repeatedly some number of times on a single element of the set.  This is an abstraction of the idea of modular exponentiation, raising some number to a power.  And we can also reverse exponentiation, though as before, this requires brute force.  For a variety of technical reasons, we normally use elliptic curves as our group set, and elliptic curve addition as our group operation.

For a finite group, the number of elements in the set is called the group $order$.  Each element also has an order, which is the number of times you can apply exponentiation before you return to the original element.  Elements whose order is equal to the group order are called $generators$, because exponentiating them will produce every element of the group; groups with at least one generator we call $cyclic$.  If the order of a group is prime, then all non-identity elements are generators.

Since exponentiation in these groups is cyclic, there is no need for an exponent to be larger than the group order.  So we use numbers modulo the group order for this operation; these numbers we call group scalars.
...
We use lowercase variable names for group scalars ($x$), and uppercase for group elements ($X$).  Groups themselves will be denoted as $\mathbb{G}$, which we will also use for sets as in $\mathbb{S}$.  Though we will be expressing all of the math here in terms of exponentials, we borrow these naming conventions from elliptic curves to make things explicit.  A public key $X$ is defined as $G^x$ for some generator $G$ and some private key $x$.
...
When dealing with groups, we often want to hash some data, then convert the ouput bits to a group element or scalar.  This is trivial in the case of a scalar; simply hash to the same number of bits as the scalar, then reduce modulo the group order.  We denote this function as $H_s$.

And for some groups, like numbers modulo a prime, this works for the group elements as well.  But for other groups, like elliptic curve points, there is no direct way to convert the output bits to a group element.  So algorithms had to be developed for every useful group which take the hash output and convert it to a group element.  We denote this function $H_g$.

One trivial way of defining $H_g$ over some binary data $d$ would be to use a generator $G$:
\begin{align}
  H_g(d) = G^{H_s(d)}\nonumber
\end{align}

This is problematic, though, because now there is a well known discrete logarithm for $H_g(d)$, namely $H_s(d)$.  The existence of this discrete logarithm breaks the assumptions needed to prove the security of algorithms that require $H_g$.  So we must use more complex algorithms; the details are out of scope for this document, but are well known and easily found.

\section{
  Schnorr Proofs and the Fiat Shamir Transform
}

Schnorr proofs \cite{schnorr} allow the owner of a private key to prove knowledge of it to someone who knows the corresponding public key.  It is an interactive 3-move protocol, i.e. a Sigma protocol.  But like all Sigma protocols, it can be made non-interactive using the Fiat-Shamir transform \cite{fiatshamir}.  This allows the protocol to function as a signature.

Let $G$ be a generator in a finite group $\mathbb{G}$ of prime order $p \in \mathbb{P}$, with private key $x \in \mathbb{Z}_p$, and public key $X = G^x$. The prover chooses a random $r \in Z_p$, then sets $R = G^r$.

In the interactive version of the protocol, the prover sends $R$ to the verifier, who responds with the challenge $c$.  This way the prover cannot choose $r$ after seeing $c$, which would allow the prover to cheat.  To make this non-interactive, Fiat and Shamir proposed to choose $c$ based on a hash including $R$, which prevents the prover from cheating, since $c$ depends on $R$:
\begin{align}
  c = H_s(G,X,R)
\end{align}

The prover then defines the response $s$ as below, and calculates it from $(r, c, x)$:
\begin{align}
  s = r - cx\nonumber
\end{align}
...
We can use this derived expression for $r$ to expand $R$:
\begin{align}
  R = G^r= G^{s + c x} = G^s G^{cx} = G^s G^{xc} = G^s (G^x)^c = G^s X^c
\end{align}

So if $R = G^s X^c$, then the prover must have known $x$.  Otherwise the prover would not have been able to construct a proper $s$, allowing the equation to balance.  The tuple $(R,s)$ is thus proof of knowledge of $x$.

\newpage
\section{Digital Signatures}
A Schnorr proof allows us to prove ownership of the private key $x$ for a public key $X = G^x$, but what if we want to use that key to sign some message $M$?  This message could be anything, such as an email or a cryptocurrency transaction, but generally will be the hash of some data $D$.  We must tie the message to the proof in order to form a signature.

To do this, we simply add the message $M$ when constructing the hash $c$:
\begin{align}
  c = H_s(G, X, R, M)
\end{align}

Since the hash will depend on the message, attempting to verify the proof with a different message $M_k$ will fail.  This allows us to use a Schorr proof to sign messages.

\subsection{Ring signatures}

A digital signature allows us to sign a message with a key, and the signature can be verified with the corresponding public key.  But the signature is tied to a single key, which is bad for privacy.  It would be better to construct signature that can be verified by a set of public keys $(X_1, ..., X_N)$, proving that we knew the private key $x$ for one of the them. Such a signature is called a ring signature \cite{cryptonote}.

Start with a message $M$ and public key $X_1$ for which the prover knows the private key $x$, and as before, choose a random $r \in \mathbb{Z}_p$. To mix in an additional key $X_2$ whose private key $x_2$ is unknown, select a random $s_2$ and $c_2$.  The prover sets $c_1$ as follows, using the values known to calculate it:
\begin{align}
  c_1 = H_s(G^r, G^{s_2} X_2^{c_2}, M) - c_2
\end{align}

This is equivalent algebraically to:
\begin{align}
  c_1 + c_2 = H_s(G^r, G^{s_2} X_2^{c_2}, M)\nonumber
\end{align}

Now that the prover has $c_1$, $s_1$ is as before:
\begin{align}
  s_1 = r - c_1 x\nonumber
\end{align}

The prover then sends $(X_1, s_1, c_1, X_2, s_2, c_2)$ to the verifier, who then calculates the hash
\begin{align}
  H_s(G^{s_1}  X_1^{c_1}, G^{s_2} X_2^{c_2}, M)\nonumber
\end{align}

Since $G^r = G^{s_1} X_1^{c_1}$, the two hashes are identical.  Thus 
\begin{align}
  c_1 + c_2 = H_s(G^{s_1} X_1^{c_1}, G^{s_2} X_2^{c_2}, M)
\end{align}

So if $\sum c_i$ is equal to the hash, then the prover must have known $x_i$ for some $X_i$.  And since the prover can put the real $(X,s,c)$ in either position, there is no way for the verifier to know which key was actually signed.

We can do the same for any number of additional public keys $X_k$: choose a random $(s_k,c_k)$ which is added to the hash in the form of $G^{s_k} X_k^{c_k}$ then subtracting the new $c_k$ from the hash to form the real $c_1$. Then $\sum c_i$ will be equal to the hash for both prover and verifier.  Again, the real $(X, s, c)$ can be in any position, so the proof leaks no information about which key was used to sign.
...
We then add a term to the hash for each $X$ that uses the key image $I$.  For the real $X$, we know that
\begin{align}
  H_g(X)^r &= H_g(X)^{s + cx}\nonumber\\
         &= H_g(X)^s (H_g(X)^{xc}\nonumber\\
         &= H_g(X)^s (H_g(X)^x)^c\nonumber\\
...
Putting the real $X_1$ first, the prover hash becomes:
\begin{align}
  H_s(G^r, H_g(X_1)^r, G^{s_2} X_2^{c_2}, H_g(X_2)^{S_2} I^{c_2}, M)
\end{align}

We define $c$ as before, then solve for the hash:
\begin{align}
  c_1 &= H_s(G^r, H_g(X_1)^r, G^{s_2} X_2^{c_2}, H_g(X_2)^{s_2} I^{c_2}, M) - c_2 \nonumber \\
  c_1 + c_2 &= H_s(G^r, H_g(X_1)^r, G^{s_2} X_2^{c_2}, H_g(X_2)^{s_2} I^{c_2}, M)\nonumber
\end{align}

So as before, $\sum c$ is equal to the hash.  The prover now sends $(I, X_1, s_1, c_1, X_2, s_2, c_2)$ to the verifier, who can check that their hash is still equal to $\sum c$:
\begin{align}
  c_1 + c_2 = H_s(G^{s_1} X_1^{c_1}, H_g(X_1)^{s_1} I^{c_1}, G^{s_2} X_2^{c_2}, H_g(X_2)^{s_2} I^{c_2}, M)
\end{align}

Since the key image $I$ is now tied to the signature, any attempt to sign two different ring signatures with the same $X$ will result in the same $I$, so preventing double spends is easy.  You just keep track of which key images have previously been used, and reject any new signatures with a previously used $I$.
...
Consider a set of public keys $X_i, i \in \{0, …, n-1\}$ with a secret index $j$ which denotes the public key to which we know the corresponding private key $x$.  As before, choose random $r$, and $s_i$ $\forall i \ne j$.  As with linkable signatures, define $I = H_g(X_j)^x$. Then define
\begin{align}
      L_j &= G^r\\
      R_j &= H_g(X_j)^r\\
  c_{j+1} &= H_s(L_j,R_j, M)
\end{align}

For the remaining $i \in \{j+1, …, n, …, j-1\}$ ($\mod{n}$ so $n$ goes to zero, then back to $j$)
\begin{align}
  L_i &= G^{s_i} {X_i}^{c_i}\\
  R_i &= H_g(X_i)^{s_i} I^{c_i}\\
  c_{i+1} &= H_s(L_i,R_i, M)\\
  &... \nonumber\\
  c_j &= H_s(L_{j-1}, R_{j-1}, M)
\end{align}
...
Then we can calculate $L_j$ and $R_j$ as we did for all $L_i$ and $R_i$, using $r_j$ and $c_j$, and the values should be equal to the original $L_j$ and $R_j$ calculated using $r$. The prover now sends $(I, X_0, s_0, …, X_{n-1}, s_{n-1}, c_0)$ to the verifier.  The verifier can reconstruct all $(c_i, L_i, R_i)$ and then check that $c_0 = c_n$.


\subsection{Multilayered LSAG Signatures}

LSAG signatures allow a compact representation of a linked ring signature, but if we need to sign $m$ keys each needs its own signature. MLSAG solves this by using vectors of size $m$ \cite{ringct}.  So $(I, X_i, s_i, r)$, are now:
\begin{align}
  I &= (I_{0}, ..., I_{m-1})\nonumber\\
  X_i &= (X_{i,0}, ..., X_{i,m-1})\nonumber\\
  s_i &= (s_{i,0}, ..., s_{i,m-1})\nonumber\\
...
As before, there exists a secret index $j$, for which we know the private keys to each public key. $\forall i \ne j$, $s_i$ consists of random numbers.

The prover proceeds as with an LSAG, starting with index $j$.  The $L_{j,k}$ and $R_{j,k}$ entries are calculated in the same way, but using $X_{j,k}$ and $r_k$.  $c_{j+1}$ is calculated using $L_{j,k}$ and $R_{j,k}$ for all $k \in (0, ..., m-1)$:
\begin{align}
  L_{j,k} &= G^{r_k}\\
  R_{j,k} &= H_g(X_{j,k})^{r_k}\\
  c_{j+1} &= H_s(L_{j,0}, R_{j,0}, ..., L_{j,m-1}, R_{j,m-1}, M)
\end{align}

As before, the prover calculates the remaining $L_i$ and $R_i$ using the corresponding $c_i$, $s_i$, and $I$, and the hash for $c_{i+1}$ contains the full set of $L_{i,k}$ and $R_{i,k}$:
\begin{align}
  L_{i,k} &= G^{s_{i,k}} X_{i,k}^{c_i}\\
  R_{i,k} &= H_g(X_{i,k})^{s_{i,k}} I^{c_i}\\
  c_{i+1} &= H_s(L_{i,0}, R_{i,0}, ..., L_{i,m-1}, R_{i,m-1}, M) \\
  &... \\
  c_j &= H_s(L_{j-1,0}, R_{j-1,0}, ..., L_{j-1,m-1}, R_{j-1,m-1}, M) 
\end{align}

Now that we have $c_j$, we can calculate the $s_{j,k}$ as before:
\begin{align}
  s_{j,k} = r_k - c_j x_k\nonumber
\end{align}

The prover sends similar $(I, X_0, s_0, ..., X_{m-1}, s_{m-1}, c_0)$ as with LSAG, but each $X_i$ and $s_i$ is now a vector.  The verifier again calculates the full set of $c_i$, and checks that $c_0 = c_n$.


\section{CryptoNote}

CryptoNote \cite{cryptonote} is a privacy focused cryptocurrency system which uses the linkable ring signatures explored above, plus a novel method for tying long term public keys to one time addresses.  CryptoNote is defined to use elliptic curves rather than exponentials, but we will break with that to maintain consistency with the rest of the document, and use exponentials in this section.


\subsection{One time addresses}

A CryptoNote key consists of a pair of public keys $(A, B)$ with their associated private keys $(a, b)$.  To construct a one time address, the creator of a transaction starts with a transaction private key $r$, and associated transaction public key $R$:
\begin{align}
  R=G^r\nonumber
\end{align}

The creator then uses the transaction private key $r$ with the destination public key $(A, B)$ to construct the one time address $X$.  This address is a public key, with associated private key $x$:
\begin{align}
  X = G^x = G^{H_s(rA)} B
\end{align}

A private key $(a, b)$ can then be used to scan one time address $X$ to determine if public key $(A, B)$ was the destination key.  First use $R$ and $(a, b)$ to recover the one time private key $y$:
\begin{align}
  y = H_s(Ra) + b
\end{align}
...
If $Y = X$, then public key $(A, B)$ was the same one used to construct the one time address, and the key owner is the owner of the one time address.  The owner now knows the private key $x = y$, so he is able to sign with this key in a ring signature, allowing him to spend $X$.


\subsection{View keys}

One time addresses allow a user to find the transaction outputs which he owns.  But this requires the user to scan the entire blockchain, looking at every output.  For users with limited storage and bandwidth, such as mobile clients, this can be problematic.  Such users may preferable to allow a trusted node to do the scanning for the them, while preventing the node from being able to spend the output.
...
If $Y = X$, then the node knows that the one time key belongs to the user, and returns it.


\subsection{Ring signatures}

CryptoNote uses linkable ring signatures, which sign a transaction prefix.  This is defined to be all of the transaction data except for the ring signatures themselves, including the input and output public keys, amounts, and the transaction public key.  The transaction prefix is serialized then hashed to create the signed message $M$.
...
\begin{align}
  \left<\textbf{a}_L, 2^n\right> = v ;\quad \textbf{a}_L \circ  \textbf{a}_R = \textbf{0}^n ;\quad \textbf{a}_R = \textbf{a}_L - \textbf{1}^n
\end{align}
...
\begin{align}
  \left<\textbf{a}_L, 2^n\right> = v ;\quad \left<\textbf{a}_L, \textbf{a}_R \circ y^n\right> = 0 ;\quad \left<\textbf{a}_L - \textbf{1}^n - \textbf{a}_R, \textbf{y}^n\right> = 0
\end{align}
...
\begin{align}
  \left<\textbf{a}_L - z \cdot \textbf{1}^n, z^2 \cdot \textbf{2}^n + z \cdot \textbf{y}^n + \textbf{a}_R \circ \textbf{y}^n\right> &= z^2 v + z \cdot \left<\textbf{1}^n, \textbf{y}^n\right> + \left<-z \cdot \textbf{1}^n, z^2 \cdot \textbf{2}^n + z \cdot \textbf{y}^n\right>\nonumber\\
  ...
  \left<\textbf{a}_L - z \cdot \textbf{1}^n, z^2 \cdot \textbf{2}^n + z \cdot \textbf{y}^n + \textbf{a}_R \circ \textbf{y}^n\right> &= z^2 v + z \cdot \left<\textbf{1}^n, \textbf{y}^n\right> - z^3 \cdot \left<\textbf{1}^n, \textbf{2}^n\right> - z^2 \cdot \left<\textbf{1}^n, \textbf{y}^n\right>\nonumber\\
...
\begin{align}
  X_i = ((X_{i,0}, C_{i,0}), ..., (X_{i,m-1}, C_{i,m-1}))
\end{align}
...
\begin{align}
  c_{j+1} = H_s(L_{j,0}, L_{j,0}, ... , L_{j,0}, L_{j,0}, G^{s_{j,m}} X^{c_j})
\end{align}
...
There is a new Zerocoin based cryptocurrency system called Lelantus \cite{lelantus}, released 2018-12-22.  It uses confidential transactions, and due to the logarithmically scaling nature of the proofs, can mixin a very large number of inputs while keeping the proof size small.  It is still necessary to have input commitments in order to verify, but these can be entire blocks, which make network analysis sufficiently difficult that it becomes thermodynamically impossible.

As per Zerocoin, Lelantus uses the output commitments as the transaction inputs/outputs themselves, rather than associating the commitments with a one time address as in RingCT.  It is thus necessary to reveal the commitment secret during the spend, similar to the way that CryptoNote reveals the key image.  So it is necessary to make the commitments double blind, or else after secret reveal it would be possible to brute force the values as per the naive approach to Pedersen commitments.
...
Ring signatures allow hiding a signature in a set of mixins.  But they grow in size linearly with the mixin set size.  So the mixin set must be limited, and cannot contain every transaction in the ledger.  Merkle proofs offer a logarithmic sized proof for ledger inclusion, but are not zero knowledge.  It would be ideal to be able to demonstrate both ownership and ledger inclusion with a single proof.  One out of many proofs do this, in logarithmic size.  So every output in the ledger functions as a mixin, and the proof size scales logarithmically with the ledger size.
...
Lelantus accomplishes this via a two step process.  It establishes input ownership via a set of one out of many $\Sigma$-proofs, then uses elements of the $\Sigma$-proofs to establish a balance proof.  At no time are the input commitments themselves revealed to the verifier.
...
The transaction format is thus the set $(C_{i_1}, ..., C_{i_N}, C_{o_1}, ..., C_{o_M}, C_T)$, with a signature on $C_T$ to prove the balance.  Given this format, it is trivial to combine transactions; you can simply add the new input, output, and balance commitments to the existing set, and the balance check should still succeed with the combined sets:
\begin{align}
